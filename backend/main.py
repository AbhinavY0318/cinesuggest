...
# FASTAPI APP
# =========================
app = FastAPI(title="CineSuggest API", version="1.0")
...
async def route_hybrid(title: str, top_n: int = 10):
    ...
    out = []
    for t, s, mood_m, topic_m in recs:
        card = await tmdb_card_from_title(t)
        out.append(HybridRecItem(title=t, score=s, mood_match=mood_m, topic_match=topic_m, tmdb=card))
    return out
...
async def movie_bundle(query: str, tfidf_top_n: int = 8, hybrid_top_n: int = 8, genre_limit: int = 8):
    ...
    try:
        tfidf_raw = tfidf_recommend(title, tfidf_top_n)
    except:
        tfidf_raw = []
    ...
    try:
        hybrid_raw = hybrid_recommend(title, hybrid_top_n)
    except:
        hybrid_raw = []

    hybrid_items = []
    for t, s, mood_m, topic_m in hybrid_raw:
        card = await tmdb_card_from_title(t)
        hybrid_items.append(HybridRecItem(title=t, score=s, mood_match=mood_m, topic_match=topic_m, tmdb=card))

    genre_recs = []
    if details.genres:
        gid = details.genres[0]["id"]
        discover = await tmdb_get("/discover/movie", {"with_genres": gid, "language": "en-US", "sort_by": "popularity.desc", "page": 1})
        for m in discover.get("results", [])[:genre_limit]:
