...
class SearchBundleResponse(BaseModel):
    ...
    mood_recommendations: List[TMDBMovieCard]
    topic_recommendations: List[TMDBMovieCard]
# =========================
# UTILS
# =========================
...
def tfidf_recommend_titles(
    query_title: str, query_mood: Optional[str] = None, query_topic_id: Optional[int] = None, top_n: int = 10
) -> List[Tuple[str, float]]:
    """
    Returns list of (title, score) from local df using cosine similarity on TF-IDF matrix.
    Safe against missing columns/rows.
    Modified to boost scores if mood or topic_id matches (along with TF-IDF).
    """
    ...
    if query_mood and 'mood' in df.columns:
        for i in range(len(scores)):
            movie_mood = df.iloc[i]['mood']
            if movie_mood == query_mood:
                scores[i] += 0.2  # Adjustable boost factor
    # Boost if topic_id matches
    if query_topic_id is not None and 'topic_id' in df.columns:
        for i in range(len(scores)):
            movie_topic = df.iloc[i]['topic_id']
            if movie_topic == query_topic_id:
                scores[i] += 0.2  # Adjustable boost factor
    # sort descending
...
def load_pickles():
    ...
    # Load df (assuming now includes 'mood' and 'topic_id' from notebook)
    ...
    if 'mood' not in df.columns:
        print("Warning: 'mood' column not found in df.pkl - mood features disabled")
    if 'topic_id' not in df.columns:
        print("Warning: 'topic_id' column not found in df.pkl - topic features disabled")
# =========================
# ROUTES
# =========================
@app.get("/health")
...
async def recommend_tfidf(
    ...
    if 'mood' in df.columns:
        query_mood = df.iloc[get_local_idx_by_title(title)]['mood']
    if 'topic_id' in df.columns:
        query_topic_id = df.iloc[get_local_idx_by_title(title)]['topic_id']
    recs = tfidf_recommend_titles(title, query_mood, query_topic_id, top_n=top_n)
...
async def search_bundle(
    ...
    tmdb_id = int(best["id"])
    details = await tmdb_movie_details(tmdb_id)
    # Compute query mood and topic (from local if possible, else fallback for mood)
    query_mood: Optional[str] = None
    query_topic_id: Optional[int] = None
    local_idx: Optional[int] = None
    try:
        local_idx = get_local_idx_by_title(details.title)
        if 'mood' in df.columns:
            query_mood = df.iloc[local_idx]['mood']
        if 'topic_id' in df.columns:
    ...
    try:
        ...
        recs = tfidf_recommend_titles(details.title, query_mood, query_topic_id, top_n=tfidf_top_n)
    except Exception:
        # fallback to user query
        try:
            recs = tfidf_recommend_titles(query, query_mood, query_topic_id, top_n=tfidf_top_n)
        except Exception:
            recs = []
    ...
    # 3) Mood recommendations (local df filtered by mood, sorted by popularity)
    mood_recs: List[TMDBMovieCard] = []
    if query_mood and 'mood' in df.columns:
        mood_df = df[df['mood'] == query_mood].sort_values('popularity', ascending=False)
        mood_titles = mood_df['title'].tolist()[:genre_limit]  # Reuse limit
        for t in mood_titles:
            if _norm_title(t) == _norm_title(details.title):
                continue
            card = await attach_tmdb_card_by_title(t)
            if card:
                mood_recs.append(card)
    # 4) Topic recommendations (local df filtered by topic_id, sorted by popularity)
    topic_recs: List[TMDBMovieCard] = []
    if query_topic_id is not None and 'topic_id' in df.columns:
        topic_df = df[df['topic_id'] == query_topic_id].sort_values('popularity', ascending=False)
        topic_titles = topic_df['title'].tolist()[:genre_limit]  # Reuse limit
        for t in topic_titles:
            if _norm_title(t) == _norm_title(details.title):
                continue
            card = await attach_tmdb_card_by_title(t)
            if card:
                topic_recs.append(card)
    return SearchBundleResponse(
        query=query,
        movie_details=details,
        tfidf_recommendations=tfidf_items,
        genre_recommendations=genre_recs,
        mood_recommendations=mood_recs,
